...
Vagrant.configure("2") do |config|
  ...
  config.vm.define 'esmaster' do |esmaster|
    esmaster.vm.network "private_network", ip: "192.168.33.19", virtualbox__intnet: true
  ...
  (0..9).each do |es_index|
    config.vm.define "es#{es_index}" do |es|
      es.vm.network 'private_network', ip: "192.168.33.2#{es_index}", virtualbox__intnet: true
  ...
  config.vm.define "core" do |core|
    ...
    core.vm.network "private_network", ip: "192.168.33.10", virtualbox__intnet: true
  ...
  config.vm.define "db" do |db|
    db.vm.network "private_network", ip: "192.168.33.11", virtualbox__intnet: true
